...
4. Dependency mechanism (Section 9): to be finalized once Android project structure is scaffolded — expected shape is a Gradle module/library dependency (native equivalent of "package"), confirmed explicitly before any paid-tier code is written, per Section 9.
...
**Map features:** lightweight world map; zoom out to spherical globe view (Ventusky-like concept); zoom in to regional/local; lat/long display + manual input; save ship's current location locally.

**Forecast system:** default period always 10 days (paid tier — free tier is 1 day per Section 1). Bottom-of-screen controls: Date / Time UTC / Latitude / Longitude — changing any updates wind, pressure systems, isobars, typhoon info, temperature accordingly.
