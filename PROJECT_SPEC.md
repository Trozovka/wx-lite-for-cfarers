...
**Weather front symbols (cold/warm/occluded/stationary) — requested, not yet built.** Deliberately not attempted with a naive heuristic (e.g. inferring fronts from temperature gradients): real frontal analysis is a nontrivial meteorological diagnostic, and a wrong front placement presented as authoritative would be actively misleading for the passage-planning decisions this app is meant to support. Needs either a genuine analyzed-front data source (e.g. NOAA/OPC surface analysis, if a machine-readable feed exists — not yet researched) or an explicit operator decision to accept a clearly-labeled approximate heuristic. Flagged rather than silently built or silently dropped.
