...
**Map features (revised 2026-08-03):** full-screen pan/zoom map (local Canvas transform, no tile-streaming basemap) with a 1-degree lat/lon grid. A fixed-center crosshair (constant screen size regardless of zoom — precision comes from zooming in, not the crosshair changing size) continuously reads out the exact lat/lon under it. The earlier "save ship's current location locally" single-point model is superseded by a **passage-plan area**: up to 10 waypoints, entered in degrees-minutes (`xx-xx.x N/S`, `yyy-yy.y E/W`, the maritime convention, not decimal degrees) on a separate entry screen, connected in order to outline the area relevant to the voyage. Point #1 sets the map's initial camera position. Sync fetches the distinct tile(s) that actually contain each passage-plan waypoint (`Tiles.tilesFor`), else the tile under the crosshair if no area is set (not the whole world at once — measured at ~155KB/region vs. ~3.6MB for all 24 tiles, a real bandwidth difference on a Marlink-class link); panning to a different region and syncing again accumulates additional cached regions rather than replacing the previous one. Two earlier approaches were tried and reverted here: syncing every tile the area's bounding box touched (pulled in tiles the area barely grazed, slower than needed) and syncing a single tile at the area's centroid (a real, device-confirmed bug — the arithmetic mean of a spread-out area can land in open ocean nowhere near any actual waypoint; caught with a real 4-point test case spanning Florida to the Irish Sea). **Known limitation:** tiles are a fixed 30°×60° grid (Section 18), so even a tile that genuinely contains a waypoint can extend well beyond a small passage-plan area, and a route spanning multiple regions still needs multiple tiles synced. A genuinely tight fit to the drawn area would need the backend to generate custom-sized regions per request instead of pre-baked fixed tiles — not done, flagged as a real structural constraint of the current architecture. The earlier spherical-globe zoom-out view (orthographic projection) was built, then removed after device testing showed it was the wrong interaction model for this app (small fixed panel, no pan/zoom) — superseded by the pan/zoom map itself.

The chart also shows a **low-pressure movement indicator**: a red arrow from each tracked low pointing toward its position at the next cached forecast hour (`LowPressureTracker`, nearest-neighbor matching between the current and next hour's detected lows within a plausible travel distance — a simple heuristic, not a real storm-tracking algorithm, adequate at the scale of one system moving between two forecast steps a few hours apart). Recomputed on every hour change, so the arrow direction updates as the user steps through the forecast, per explicit operator request. No arrow is drawn where the next hour isn't cached (can't determine movement from one snapshot) — degrades silently rather than guessing.

**Weather front symbols (cold/warm/occluded/stationary) — decided against, 2026-08-03.** Considered and explicitly dropped, not deferred: real frontal analysis is a nontrivial meteorological diagnostic (not reliably derivable from a simple heuristic on temperature/wind grids), and a wrong front placement presented as authoritative would be actively misleading for the passage-planning decisions this app is meant to support. Building it would need a genuine analyzed-front data source (e.g. NOAA/OPC surface analysis) that hasn't been researched, and the operator decided the complexity/risk isn't worth it. Not in scope going forward unless revisited explicitly.
...
**Longitude convention bug, found and fixed 2026-08-03:** GRIB2 data from NOAA/NOMADS is natively in 0–360°E longitude, but every other part of this project (`tiles.py`'s tile definitions, the Kotlin app, coastline data, user-entered coordinates) uses -180..180. `pack.py` was writing the GRIB's raw 0–360 values straight into the `.wxl` header, unconverted — for a simple Western-hemisphere tile this was a clean +360 offset (e.g. header claimed lon 240–300 instead of -120..-60); for a tile straddling the 0/360 wraparound point, naively taking `min()`/`max()` of a longitude array whose own values wrap produced a header claiming lon 0–359.75, i.e. "the whole world." The underlying weather data was correct the whole time — only the header's claimed bounds were wrong, causing wind/pressure to render in a completely different part of the world than the actual coastlines/waypoints. Never caught earlier because every previously-tested tile (Manila, Singapore) has positive longitude, where the two conventions are identical. Fixed by having `pack()` accept and use the tile's own already-known request bounds directly, rather than re-deriving bounds from the GRIB data. Backend-only fix — no Android app changes were needed, since `WxlFile` parsing always correctly trusted whatever the header said; the header was simply wrong. A full data rebuild was triggered manually via `workflow_dispatch` immediately after the fix, since the live published data predating this fix is wrong for every tile with negative longitude.
